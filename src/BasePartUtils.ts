...
export function rainbowEffect(basePart: BasePart, dur: number) {
	...
	const playNextTween = () => {
		if (basePart !== undefined && basePart.Parent !== undefined) tweens[currentIndex].Play();
		else endRainbow();
	};
...
export function isInside(position: Vector3, volumeCenter: CFrame, volumeSize: Vector3) {
	...
	return (
		math.abs(v3.X) <= volumeSize.X / 2 && math.abs(v3.Y) <= volumeSize.Y / 2 && math.abs(v3.Z) <= volumeSize.Z / 2
	);
}
...
export function isCompletelyInside(cframe: CFrame, size: Vector3, part?: BasePart) {
	...
	for (const cf of corners) {
		if (!isInsidePart(cf.Position, part)) return false;
	}
	return true;
}
