 ...
 */
export function observeTagAdded(
	tag: string,
	callback: (instance: Instance) => (() => void) | unknown,
	workspaceOnly = false,
) {
	const check = (instance: Instance) => {
		if (workspaceOnly === true && !instance.IsDescendantOf(Workspace)) return;
 ...
 */
export function observeTag(
	tag: string,
	addedCallback: (instance: Instance) => void,
	removedCallback: (instance: Instance) => void,
) {
