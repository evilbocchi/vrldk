...
export namespace Streaming {
	...
	export function fireNearby(remote?: Instance, ...args: unknown[]) {
		...
		if (parent === undefined || !parent.IsA("BasePart")) return;

		const position = parent.Position;
		const players = Players.GetPlayers();
		for (const player of players) {
			if (player.Character === undefined || player.Character.PrimaryPart === undefined) continue;
	...
	export const createStreamableRemote = (root: Model, isUnreliable?: boolean) => {
		...
		if (parent === undefined) throw "Root must be a model or part with a primary part.";
